...
void game_draw(void) {
    ...
    if (tex_night_text) {
        ...
        if (current_night >= 1 && current_night <= 5) {
            ...
            if (tex_night_num[n_idx]) {
                int num_x = night_x + 63 + 10; 
                int num_y = 72;                 
                SDL_Rect dst_num = {num_x, num_y, 14, 17};
    ...
    int tens = display_power / 10;
    int units = display_power % 10;
    ...
    // 4. Overlays (Cámara por encima de todo)
    if (cam_frame > 0.0f) {
